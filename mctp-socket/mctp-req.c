...
static int mctp_req(unsigned int net, mctp_eid_t eid,
	unsigned int ifindex, u8 *lladdr, int lladdrlen,
	u8 type, u8 *data, size_t len)
{
...
static void usage(void)
{
	...
	fprintf(stderr, " [ifindex <ifindex> lladdr <hwaddr>] [type <type>] [len <len>]");
	fprintf(stderr, " [data <data>] [skip 1] [debug 1]\n");
	fprintf(stderr, "default eid %d net %d len %zd\n",
			DEFAULT_EID, DEFAULT_NET, DEFAULT_LEN);
	fprintf(stderr, "The data is in hex format separated by colon, e.g. 00:01:0f");
}

int main(int argc, char **argv)
{
	u8 *data, lladdr[MAX_ADDR_LEN];
